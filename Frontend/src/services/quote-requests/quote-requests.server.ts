...
export const getQuoteRequestsServer = async (userId: string, authToken: string) =>
  unwrap(await serverApiClient.get<QuoteRequest[]>(`${ENDPOINTS.QUOTE_REQUESTS.LIST}?userId=${userId}`, authToken));
